...
const stats = [
  ...
  },
  {
    id: 6,
    title: "100% garantía",
    description: "en productos certificados"
  }
]
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {stats.map((stat) => (
            <div key={stat.id} data-aos="zoom-in">
              <h3 className="text-3xl md:text-4xl font-bold text-primary text-center md:text-left">
                {stat.title.split(" ").map((part, idx) =>
                  idx === 1 ? (
                    <span key={idx} className="text-blue-500">{part}</span>
                  ) : (
